...
public class AdminTestController implements Initializable {
    ...
    public void save() {
        if (txtFirstName.getText().isEmpty() || txtLastName.getText().isEmpty() || txtPhone.getText().isEmpty() || txtEmail.getText().isEmpty() || choiceBox.getSelectionModel().isEmpty()) {
    ...
    public void saveP() throws SQLException {
        if (txtTitle.getText().isEmpty() || txtDesc.getText().isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText(null);
            alert.setContentText("You have not entered data in one or more fields. Please try again.");
